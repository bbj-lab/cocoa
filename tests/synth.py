...
SCHEMAS = {
    ...
    # not referenced by the shipped default config; used by tests that exercise
...
def minimal_collation_cfg(**overrides) -> dict:
    """
    smallest workable collation config: a hospitalization reference table and
    one vitals entry, with every subject assigned to the training split so the
    learned vocabulary covers the whole dataset
    """
    cfg = {
        "subject_id": "hospitalization_id",
        "default_timezone": "UTC",
        "subject_splits": {"train_frac": 1.0, "tuning_frac": 0.0},
        "reference": {
...
def write_minimal_dataset(
    ...
    *,
    ...
    """
    write only the tables `minimal_collation_cfg` reads, from explicit rows, so
    that a test can state an exact expected timeline;

    `hospitalizations` rows need `admission_dttm` and `discharge_dttm` and
    default their ids; `vitals` rows default to a heart_rate reading;
    times are localized to `tz` unless `naive_times` asks for them to be left
    naive (to exercise the collator localizing them itself)
    """
    root = pathlib.Path(dest).expanduser().resolve()
    root.mkdir(parents=True, exist_ok=True)
    ...
    hosp = [
        {
            "hospitalization_id": h.get("hospitalization_id", f"H{n}"),
            "patient_id": h.get("patient_id", f"P{n}"),
            "admission_dttm": h["admission_dttm"],
            "discharge_dttm": h["discharge_dttm"],
            "age_at_admission": h.get("age_at_admission", 50.0),
            "admission_type_category": h.get("admission_type_category", "Inpatient"),
            "discharge_category": h.get("discharge_category", "Home"),
        }
        for n, h in enumerate(hospitalizations)
    ]
    vits = [
        {
            "hospitalization_id": e.get(
                "hospitalization_id", hosp[0]["hospitalization_id"]
            ),
            "recorded_dttm": e["recorded_dttm"],
            "vital_category": e.get("vital_category", "heart_rate"),
            "vital_value": e.get("vital_value"),
        }
        for e in vitals
    ]
    frames = {
        "clif_hospitalization": pl.DataFrame(
            hosp, schema=SCHEMAS["clif_hospitalization"], orient="row"
        ),
        "clif_patient": pl.DataFrame(
            [
                {
            ...
            schema=SCHEMAS["clif_patient"],
            orient="row",
        ),
        "clif_vitals": pl.DataFrame(vits, schema=SCHEMAS["clif_vitals"], orient="row"),
    }
    for table, df in frames.items():
        if tz is not None and not naive_times:
            df = df.with_columns(
                pl.col(c).dt.replace_time_zone(tz).alias(c)
                for c, t in SCHEMAS[table].items()
                if t == DTTM
            )
        df.write_parquet(root / f"{table}.parquet")
    return root
